...
function linkify(links) {
  ...
  return String(links)
    ...
    .map((l) => l.trim())
    .filter(Boolean)
    .map((l, i) => `<a href="${esc(l)}" target="_blank" rel="noopener">Link ${i + 1} ↗</a>`)
    .join("");
}
...
function renderHeader(activeKey, activeChild) {
  ...
  header.innerHTML = `
    <div class="nav-wrap">
      ...
      <button class="menu-toggle" id="menu-toggle" aria-label="Toggle menu"><span></span></button>
    </div>
  `;
  ...
  // Mobile: tap top-level item with children to expand submenu instead of navigating away
